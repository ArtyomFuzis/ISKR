...
import './ChangeProfilePhotoModal.scss';
...
function ChangeProfilePhotoModal({ open, onClose, currentImageUrl, onSuccess }: ChangeProfilePhotoModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Проверяем структуру ответа (может быть uploadResponse.data или uploadResponse напрямую)
      ...
      if (uploadResult.state === 'OK') {
        ...
        // 2. Обновляем фото профиля с полученным imageId
        const changeResponse = await profileAPI.changeProfileImage(imageId);
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      <div className="change-profile-photo-modal">
        <h2 className="modal-title">Смена фото профиля</h2>
        
        <div className="photo-preview-container">
          <div className="current-photo">
            <span className="preview-label">Текущее фото:</span>
            <img src={currentImageUrl} alt="Текущее фото профиля" className="preview-image" />
          </div>
          
          {previewUrl && (
            <div className="new-photo">
              <span className="preview-label">Новое фото:</span>
              <img src={previewUrl} alt="Новое фото профиля" className="preview-image" />
            </div>
          )}
        </div>
